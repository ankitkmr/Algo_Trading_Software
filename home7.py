...
def bollinger():
    ...
    dt_start = dt.datetime(2007, 12, 1)
    dt_end = dt.datetime(2010, 2, 2)
...
def find_events(ls_symbols, d_data, df_bollinger):
     ...
     for s_sym in ls_symbols:
        for i in range(1, len(ldt_timestamps)):
            ...
            f_cutoff = -2.0
            if f_bollinger_today <= f_cutoff and f_bollinger_yest >= f_cutoff and f_bollinger_market_today >= 1.3:
                 with open('orders4.csv', 'a') as fp:
		    a1 = csv.writer(fp, delimiter=',')
  		    row = [[ldt_timestamps[i] ,s_sym,'Buy',100], [ldt_timestamps[i+5],s_sym,'Sell',100]]
 	     	    a1.writerows(row)
